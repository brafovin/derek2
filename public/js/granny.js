...
class Granny {
  ...
  _build() {
    ...
    // Chainsaw
    this.chainsawGroup = new THREE.Group();
    // Handle
    const chainHandleMat = new THREE.MeshLambertMaterial({ color: 0x333333 });
    const csHandle = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.35, 0.1), chainHandleMat);
    this.chainsawGroup.add(csHandle);
    // Body
    const csBody = new THREE.Mesh(new THREE.BoxGeometry(0.15, 0.12, 0.55), chainHandleMat);
    csBody.position.set(0, 0.18, 0.2);
    this.chainsawGroup.add(csBody);
    // Blade
    const bladeMat = new THREE.MeshLambertMaterial({ color: 0x888888 });
    const csBlade = new THREE.Mesh(new THREE.BoxGeometry(0.06, 0.08, 0.5), bladeMat);
    csBlade.position.set(0, 0.18, 0.55);
    this.chainsawGroup.add(csBlade);
    // Chain teeth
    for (let i = 0; i < 8; i++) {
      ...
      tooth.position.set(0.04, 0.22, 0.25 + i * 0.07);
      this.chainsawGroup.add(tooth);
    }
    // Blood on blade
    const chainBlood = new THREE.Mesh(new THREE.BoxGeometry(0.08, 0.06, 0.3), bloodMat);
    chainBlood.position.set(0, 0.18, 0.7);
    this.chainsawGroup.add(chainBlood);

    this.chainsawGroup.position.set(0.28, 1.0, 0);
    this.chainsawGroup.rotation.x = -0.5;
    this.armR.add(this.chainsawGroup);
  ...
  update(dt, time) {
    ...
    if (this.state === 'chase') {
      ...
      this.armR.rotation.x = Math.sin(this.chainsawTimer * 15) * 0.3 - 0.5;
      this.armR.rotation.z = Math.sin(this.chainsawTimer * 8) * 0.2 - 0.4;
      this.chainsawGroup.rotation.z = Math.sin(this.chainsawTimer * 20) * 0.15;

      // Chainsaw chain vibration
      this.chainsawGroup.position.x = Math.random() * 0.02 - 0.01;
